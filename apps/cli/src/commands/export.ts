...
export const Export = Command.make(
	"export",
	{
		...
		id: Options.optional(
			Options.text("id").pipe(
				Options.withDescription("Filter by result ID")
			)
		),
		by: Options.optional(
			Options.text("by").pipe(
				Options.withDescription("Filter by worker ID")
			)
		),
