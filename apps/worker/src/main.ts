...
const program = Effect.gen(function* () {
  ...
  const [errors, workers] = yield* pipe(
    config.extractors,
    Effect.partition(
      path => Effect.tryPromise(() => import(path)).pipe(Effect.mapError(() => path)),
      {concurrency: "unbounded"}
    ),
    Effect.map(
      ([errors, workers]) =>
        [errors, Array.map(workers, module => module.default as Extractor)] as const
    )
  )
  ...
  const down = () =>
    Ref.updateAndGet(limit, n => Math.max(n / 2, 1)).pipe(Effect.tap(n => gate.resize(n)))
