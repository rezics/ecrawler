...
const ignore = (fn: Function) => async () => {
	try {
		await fn()
	} catch (e) {}
}
...
if (import.meta.main) {
	...
	try {
		...
		const DATABASE_INIT = [
			...
			// Insert tokens after migrations create the tables
		].join("\n")
		await within(async () => {
			cd("./tools/database")
			...
			cleanup.push(() => $`podman stop ${DATABASE_CONTAINER_NAME}`.nothrow().quiet() as never)
		...
		// Insert dispatcher token after migrations
		...
		// Insert collector token after migrations
		...
		await within(async () => {
			...
			const tasks = [
				{tags: ["dummy", "data"], link: "https://example.com"},
		...
		await within(async () => {
			...
			$.env["WORKERS"] = ["@ecrawler/worker-dummy/data.ts", "@ecrawler/worker-dummy/link.ts"].join(",")
			await $`yarn run -T tsx ./src/main.ts`.stdio("inherit")
		})
	} catch (e) {
		throw e
	} finally {
		title("CLEANUP")
		for (const fn of cleanup.map(ignore)) {
			await fn()
		}
	}
}
